...
fn main() {
    App::new()
        ...
        .add_systems(Update, (handle_mouse, draw_pixel_grains))
        .run();
}

const DIMENSION: i64 = 20;
...
fn draw_pixel_grains(
    ...
    //println!("pixel_grains_hash len: {}", pixel_grains_hash.len());
    //println!("staring loop, {}, {}, {}, {}", top_left.x.to_pixel_size(), botton_right.x.to_pixel_size(), top_left.y.to_pixel_size(), botton_right.y.to_pixel_size());
    for x in ((top_left.x.to_pixel_size() as i64)..(botton_right.x.to_pixel_size() as i64)).step_by(PIXEL_SIZE as usize) {
        ...
        for y in ((botton_right.y.to_pixel_size() as i64)..(top_left.y.to_pixel_size() as i64)).step_by(PIXEL_SIZE as usize)  {
            ...
            //println!("spawn loc, {}, {}", x, y);
            commands.spawn((PixelGrain::new(x as i64, y as i64), MaterialMesh2dBundle {
                mesh: shape,
